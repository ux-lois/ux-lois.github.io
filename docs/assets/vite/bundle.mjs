...
var H = 0.7, j = 1 / H, T = "\\s*([+-]?\\d+)\\s*", X = "\\s*([+-]?(?:\\d*\\.)?\\d+(?:[eE][+-]?\\d+)?)\\s*", x = "\\s*([+-]?(?:\\d*\\.)?\\d+(?:[eE][+-]?\\d+)?)%\\s*", tr = /^#([0-9a-f]{3,8})$/, er = new RegExp(`^rgb\\(${T},${T},${T}\\)$`), nr = new RegExp(`^rgb\\(${x},${x},${x}\\)$`), rr = new RegExp(`^rgba\\(${T},${T},${T},${X}\\)$`), ir = new RegExp(`^rgba\\(${x},${x},${x},${X}\\)$`), or = new RegExp(`^hsl\\(${X},${x},${x}\\)$`), sr = new RegExp(`^hsla\\(${X},${x},${x},${X}\\)$`), Et = {
  aliceblue: 15792383,
